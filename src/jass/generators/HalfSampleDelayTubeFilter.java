...
public class HalfSampleDelayTubeFilter extends HalfSampleDelayKLFilter {
     ...
     */
    public HalfSampleDelayTubeFilter(float srate, TubeShape ts,double c) {
        // figure out how many segments are needed for the filter
        super(srate,(int)(2*ts.getLength()*srate/c));
        int k=(int)(2*ts.getLength()*srate/c);
        System.out.println("TubeFilter with n="+k+"segments");
        this.c=c;
        tubeShape = ts;
        setKLRadii();
    }
    ...
    */
    protected void setKLRadii() {
        ...
        int nSections = (int)(2*tubeShape.getLength()*srate/c);
        setEnd(nSections);
        int nTubeSections = state.nSectionsUsed;
        for(int k=0;k<nTubeSections;k++) {
            ...
            setCylinderRadius(k,tubeShape.getRadius(xc));
